...
export const CACHE_API_FORMAT_INFO = {
    ...
    [CACHE_API_FORMATS.GEMINI]: {
        ...
        note: 'Gemini 2.5+ 支持隐式缓存；显式缓存会先创建 cachedContents，再在 generateContent 里引用，默认 TTL 为 1 小时。官方地址优先使用 x-goog-api-key；自定义代理地址优先使用 Authorization: Bearer，并会在鉴权错误时自动回退。部分代理不开放 cachedContents 端点，工具会自动降级为隐式缓存测试。',
    },
}
